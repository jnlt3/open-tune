...
def addTest():
    ...
    for x in range(1, parameters_number + 1):
        paramDB = Param(
            ...
            value=request.form.get("value" + x.__str__()),
            lowest=request.form.get("lowest" + x.__str__()),
            highest=request.form.get("highest" + x.__str__()),
            step=request.form.get("step" + x.__str__()),
